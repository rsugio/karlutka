...
class Hmi {
    class Instance(
        ...
        fun toHmiRequest(): HmiRequest {
            ...
            val ats = attributes.filter { it.leave_typeid == "string" }.associateBy({ it.name }, { it.value[0].text!! })
            ...
            return HmiRequest(
                typeid,
                requireNotNull(ats["ClientId"]),
                requireNotNull(ats["RequestId"]),
                level,
                inpType,
                inpm,
                requireNotNull(ats["MethodId"]),
                requireNotNull(ats["ServiceId"])
            )
        }
    }
    ...
    class HmiRequest(
        ...
        constructor(inst: Instance) : this(
            ...
            inst.attributes.find { it.name == "MethodInput" }!!.value[0].instance?.attributes?.get(0)?.value?.associate { v ->
                Pair(v.instance?.attributes?.find { it.name == "Key" }?.value?.get(0)?.text!!,
    ...
    class HmiResponse(
        ...
        constructor(inst: Instance) : this(inst.typeid,
            inst.attributes.find { it.name == "ClientId" }!!.value[0].text!!,
            inst.attributes.find { it.name == "RequestId" }!!.value[0].text!!,
        ...
        fun toInstance(): Instance {
            ...
            lst.add(Attribute("ClientId", ClientId))
            lst.add(Attribute("ControlFlag", ControlFlag))
            if (CoreException == null) lst.add(Attribute("CoreException", Value(0, true)))
            else lst.add(Attribute("CoreException", CoreException))
            lst.add(Attribute("HmiSpecVersion", HmiSpecVersion))
            if (MethodFault == null) lst.add(Attribute("MethodFault", Value(0, true)))
            else lst.add(Attribute("MethodFault", MethodFault))
            ...
            while (xr.hasNext()) {
                ...
                when {
                    xr.isStartElement() && xr.localName == "attribute" -> {
                        ...
                        la = Attribute(requireNotNull(ats["isleave"]).toBooleanStrict(), ats["leave_typeid"],
                            requireNotNull(ats["name"]))
                        lst.add(la)
                    }

                    xr.isStartElement() && xr.localName == "value" -> {
                        val ats = xr.attributes.associate { Pair(it.localName, it.value) }
                        lv = Value(requireNotNull(ats["index"]).toInt(), requireNotNull(ats["isnull"]).toBooleanStrict())
                    }
