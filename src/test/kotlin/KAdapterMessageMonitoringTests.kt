...
class KAdapterMessageMonitoringTests {
    ...
    fun stable() {
        ...
        val fault = Fault()
        val userfilter = KSoap.parseSOAP<GetUserDefinedSearchFiltersResponse>(s("/pi_AdapterMM/01.xml"), fault)
        require(userfilter!!.Response.MessageInterface.size == 3 && fault.isSuccess())
        val b = KSoap.parseSOAP<GetUserDefinedSearchFiltersResponse>(s("/pi_AdapterMM/02.xml"), fault)
        ...
        f1.composeSOAP()
        val f2 = KSoap.parseSOAP<GetUserDefinedSearchExtractorsResponse>(s("/pi_AdapterMM/03.xml"), fault)
        ...
        l = KSoap.parseSOAP<GetMessageListResponse>(s("/pi_AdapterMM/05prev.xml"), fault)
        require(l!!.Resp.afw!!.list[0].businessAttributes!!.list.size == 3)
        l = KSoap.parseSOAP<GetMessageListResponse>(s("/pi_AdapterMM/06fault.xml"), fault)
        ...
        val bykeysresp = KSoap.parseSOAP<GetMessagesByKeysResponse>(s("/pi_AdapterMM/07bykeys.xml"), fault)
        ...
        var uds2 = KSoap.parseSOAP<GetUserDefinedSearchAttributesResponse>(s("/pi_AdapterMM/08attrs.xml"), fault)
        require(uds2!!.Resp.list.isEmpty())
        uds2 = KSoap.parseSOAP<GetUserDefinedSearchAttributesResponse>(s("/pi_AdapterMM/09attrs.xml"), fault)
        ...
        var log3 = KSoap.parseSOAP<GetLoggedMessageBytesResponse>(s("/pi_AdapterMM/12bytes.xml"), fault)
        require(fault.isSuccess() && log3!!.Response.isEmpty())
        log3 = KSoap.parseSOAP<GetLoggedMessageBytesResponse>(s("/pi_AdapterMM/13bytes.xml"), fault)
        require(fault.isSuccess() && log3!!.Response.isNotEmpty())

        s = ResendMessages(ArrayOfStrings(mkey)).composeSOAP()
        require(s.isNotBlank())
        val r = KSoap.parseSOAP<ResendMessagesResponse>(s("/pi_AdapterMM/14resend.xml"), fault)
        require(r != null && fault.isSuccess())
        s = CancelMessages(ArrayOfStrings(mkey)).composeSOAP()
        require(s.isNotBlank())
        val cr = KSoap.parseSOAP<CancelMessagesResponse>(s("/pi_AdapterMM/15cancel.xml"), fault)
        ...
        require(s.isNotBlank())
        var uds3 = KSoap.parseSOAP<GetUserDefinedSearchMessagesResponse>(s("/pi_AdapterMM/16uds.xml"), fault)
        require(uds3!!.Resp.number == 0 && fault.isSuccess())
        uds3 = KSoap.parseSOAP<GetUserDefinedSearchMessagesResponse>(s("/pi_AdapterMM/17uds.xml"), fault)
        ...
        require(s.isNotBlank())
        var ids = KSoap.parseSOAP<GetMessagesByIDsResponse>(s("/pi_AdapterMM/18ids.xml"), fault)
        require(!fault.isSuccess() && fault.faultstring.startsWith("Cannot find the required parameter [referenceIds]") && ids == null)
        ids = KSoap.parseSOAP<GetMessagesByIDsResponse>(s("/pi_AdapterMM/19ids.xml"), fault)
        require(fault.isSuccess() && ids!!.Resp.number == 0)
        ids = KSoap.parseSOAP<GetMessagesByIDsResponse>(s("/pi_AdapterMM/20ids.xml"), fault)
        ...
        require(s.isNotBlank())
        val sbr = KSoap.parseSOAP<GetMessageBytesJavaLangStringBooleanResponse>(s("/pi_AdapterMM/21sbr.xml"), fault)
        ...
        require(s.isNotBlank())
        var fail = KSoap.parseSOAP<FailEoioMessageResponse>(s("/pi_AdapterMM/23fail.xml"), fault)
        require(fault.isSuccess() && fail!!.Response.resultCode == "02")
        fail = KSoap.parseSOAP<FailEoioMessageResponse>(s("/pi_AdapterMM/24fail.xml"), fault)
        require(!fault.isSuccess() && fail == null)

        s = GetAllAvailableStatusDetails(LocaleAMM("en")).composeSOAP()
        require(s.isNotBlank())
        val ga = KSoap.parseSOAP<GetAllAvailableStatusDetailsResponse>(s("/pi_AdapterMM/25statuses.xml"), fault)
        require(ga!!.Response.list.size == 185 && ga.Response.list[0].errorLabelID == "6401")

        s = GetConnections().composeSOAP()
        require(s.isNotBlank())
        val conns = KSoap.parseSOAP<GetConnectionsResponse>(s("/pi_AdapterMM/26conns.xml"), fault)
        require(conns!!.Response.strings.size == 29 && conns.Response.strings[0] == "AFW")

        s = GetErrorCodes(6401).composeSOAP()
        require(s.isNotBlank())
        val ec = KSoap.parseSOAP<GetErrorCodesResponse>(s("/pi_AdapterMM/27error.xml"), fault)
        require(ec!!.Response.strings.size == 1 && ec.Response.strings[0] == "ODATA_INITIALIZATION_ERROR")

        s = GetIntegrationFlows("EN").composeSOAP()
        require(s.isNotBlank())
        val flows = KSoap.parseSOAP<GetIntegrationFlowResponse>(s("/pi_AdapterMM/28flows.xml"), fault)
        ...
        require(s.isNotBlank())
        val iface = KSoap.parseSOAP<GetInterfacesResponse>(s("/pi_AdapterMM/29iface.xml"), fault)
        require(iface!!.Response == null)

        s = GetMessagesWithSuccessors(ArrayOfStrings("7b761582-1227-11ed-8bad-0000004c2912")).composeSOAP()
        require(s.isNotBlank())
        val suc = KSoap.parseSOAP<GetMessagesWithSuccessorsResponse>(s("/pi_AdapterMM/30suc.xml"), fault)
        require(fault.isSuccess() && suc!!.Resp.afw!!.list.size == 1)

        s = GetParties().composeSOAP()
        require(s.isNotBlank())
        val part = KSoap.parseSOAP<GetPartiesResponse>(s("/pi_AdapterMM/31part.xml"), fault)
        ...
        require(s.isNotBlank())
        val pred = KSoap.parseSOAP<GetPredecessorMessageIdResponse>(s("/pi_AdapterMM/32pred.xml"), fault)
        require(pred!!.Response == null && fault.isSuccess())

        s = GetServices().composeSOAP()
        require(s.isNotBlank())
        val serv = KSoap.parseSOAP<GetServicesResponse>(s("/pi_AdapterMM/33serv.xml"), fault)
        ...
        require(s.isNotBlank())
        val stat = KSoap.parseSOAP<GetStatusDetailsResponse>(s("/pi_AdapterMM/34stat.xml"), fault)
