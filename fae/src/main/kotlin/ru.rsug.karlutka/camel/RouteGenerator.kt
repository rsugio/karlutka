...
class RouteGenerator(private val ico: XICache.AllInOneParsed) {
    val log = StringBuilder()   //лог принятия решения
    private val camelSender = ico.senderAttr.filter { it.Namespace == "camel" }.associate { Pair(it.Name, it.valueAsString()) }
    var xmlDsl = ""
    ...
    fun convertIco(): String {
        ...
        val descr = CamelDSL.Description()
        val route = CamelDSL.Route(ico.routeId, descr)
        route.add(CamelDSL.From(fromUrl, true, ico.key))
        ...
        class Tmp(
            ...
            var route: CamelDSL.Route,   // для нескольких получателей
        )
        ...
        ico.condgroups.forEachIndexed { cx, cg ->
            val tmp = Tmp(
                cg.id, "icord$cx", cg.receivers.isNotEmpty(), cg.receivers,
                CamelDSL.Route("${ico.routeId}_$cx", CamelDSL.Description("непустой"))
            )
            tmp.route.add(CamelDSL.From("direct:${ico.routeId}_$cx"))
            if (tmp.isRD)
                tmpRDs.add(tmp)
            else
                tmpIDs.add(tmp)
            ...
            if (cg.receivers.isNotEmpty()) {
                val sp = CamelDSL.SetProperty(tmp.pname, CamelDSL.Predicate.XPath("BOOLEAN", xp))
                route.add(sp)
            }
        }
        ...
        if (tmpRDs.isEmpty()) {
            if (ico.receivers.size == 1) {
                ...
                route.add(CamelDSL.To(recv.camel["connection"]!!))
            } else {
                log.append("RD содержит несколько безусловных получателей - делаем .multicast(parallel)\n")
                val multi = CamelDSL.Multicast(false)
                route.add(multi)
                ico.channelsR.forEach { recv ->
                    val rn = "${recv.receiver.party}|${recv.receiver.service}"
                    multi.children.add(CamelDSL.To(recv.camel["connection"]!!))
                }
            }
        } else {
            log.append("RD содержит условия - //TODO\n")
            route.add(CamelDSL.To("log:${ico.routeId}"))
        }
